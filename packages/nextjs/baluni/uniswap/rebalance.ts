...
export async function rebalancePortfolio(
  ...
  const _usdBalance = await getTokenBalance(dexWallet, dexWallet.walletAddress, usdcAddress);
  ...
  for (const token of desiredTokens) {
    ...
    const tokenValue = await getTokenValue(tokenSymbol, token, tokenBalance, decimals, usdcAddress);
